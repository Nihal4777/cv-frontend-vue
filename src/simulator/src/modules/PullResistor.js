...
export default class PullResistor extends CircuitElement {
    constructor(x, y, scope = globalScope, dir = 'RIGHT', bitWidth = 1,pullDirection) {
        super(x, y, scope, dir, bitWidth)

        this.rectangleObject = false;
        this.fixedBitWidth = true;
        this.setDimensions(10, 30)
        this.pullDirection = pullDirection || 0;
    ...
    resolve() {
        if (this.inp.value == undefined) {
            this.inp.value = this.pullDirection
...
changePullDirection(pullDirection) {
    ...
    const newDirection = directionMap[pullDirection];

    if (newDirection !== undefined && this.pullDirection !== newDirection) {
        this.pullDirection = newDirection;
        var obj = new PullResistor(this.x, this.y, this.scope, this.dir, this.bitWidth, this.pullDirection)
        this.delete()
